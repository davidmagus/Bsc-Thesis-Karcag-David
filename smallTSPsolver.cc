...
namespace STSP
{
#pragma region Reszfeladat_sturuktura
    struct task
    {
        int mask;
        int node;
...
#pragma region Logging_eszkozok
    struct Logging
    {
...
#pragma endregion
    ...
    template <typename Keytype = double, typename Itemtype = task>
    class Binary_heap
    {
    private:
        const int Heap_degree;
        vector<Keytype> keys; // 0. csúcs gyökér. i. csúcs gyereke:
    ...
    constexpr int approx(double Alpha) { return Alpha * 10000; }
    template <typename DEBUG = Silent, typename LB = bound::SST, int alpha_approx = 10000> // Template parameters: <DEBUG = STSP::Silent / STSP::Logging, LB = bound::BF / bound::AP / bound::SST, alpha_approx = STSP::approx(double Alpha)> parameters: const ListDigraph& _G, const ListDigraph::NodeMap<int>& _Label, const ListDigraph::ArcMap<double>& _weight, const double _Upper_bound
    class STSPsolver
    {
    private:
#pragma region adattagok
        // Kapott adatok
        const double approx_ratio = alpha_approx / 10000;
        int n;
        const ListDigraph &G;
...
#pragma endregion
        ...
        {
            // init
            int firstmask = (1 << n) - 2; // összes csúcs
            ...
            while (!tasklist.empty())
            {
                ...
                for (ListDigraph::OutArcIt a(G, V[current_task.node]); a != INVALID; ++a)
                {
                    ...
                    if (current_task.mask & (1 << Label[G.target(a)]))
                    {
                        ...
                        else if ((current_task.Lbound + (weight)[a] + Lbounds[next_mask]) * approx_ratio <= Upper_bound)
                        {
                            ...
                            if ((current_task.Lbound + (weight)[a] + Lbounds[current_task.mask]) * approx_ratio <= Upper_bound)
                            {
...
#pragma region futatas
    ...
    // timer.restart();
    // STSP::STSPsolver<STSP::Logging, bound::BF> ALG_BF(G, Label, weight);
    ...
    timer.restart();
    STSP::STSPsolver<STSP::Silent, bound::SST> SST(G, Label, weight);
    ...
    // timer.restart();
    // STSP::STSPsolver<STSP::Silent, bound::AP> ALG_AP(G,Label,weight);
    // std::cout << "Opt value: " << ALG_AP.solve() << " Best route: ";
    // ALG_AP.printroute();
    // cout << "Time: " << timer.realTime() << endl;

    // std::cout << "\n"
    //           << "2 közelítő STT Futás kezdete..." << endl;
    // timer.restart();
    // STSP::STSPsolver<STSP::Silent, bound::SST, STSP::approx(2)> SST_approx(G, Label, weight);
    // std::cout << "2 közelítő value: " << SST_approx.solve() << " Best route: ";
    // SST_approx.printroute();
    // cout << "Time: " << timer.realTime() << endl;
#pragma endregion
}