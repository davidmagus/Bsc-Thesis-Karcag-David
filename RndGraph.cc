...
int main()
{
    ...
    std::cout << "Input the Graph type: DG (Digraph), TSPpartial , G (Graph default), Ship_problem SP (int k: capacity, int n: number of cities, vector<vector<int>> C: c_ij == cost of a c_ij ticket, vector<vector<int>> D: d_ij == Max tickets that can be sold Btween ij): \n";
    std::cin >> typeofgraph;

#pragma region TSP
    if (typeofgraph == "TSPpartial") {
        int n;
        double mratio;
...
#pragma region TSPcomplete
    if (typeofgraph == "TSPcomplete") {
        int n;
        double mratio;
        cout << "Number of nodes (n): ";
        cin >> n;
