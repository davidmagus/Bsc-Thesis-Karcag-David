...
#pragma region Mincost_tree_bound
    ...
    double SSTbound(const DG& G, const maptype& weight){
    ...
    {
        template <typename DG, typename maptype>
        static double calculate(const DG& G, const maptype& weight){
            return SSTbound<DG>(G, weight);
        }
    };
#pragma endregion
...
#pragma region Assigment_matching_problem_bound
        ...
        template<typename DG, typename maptype>
    double APbound(const DG& G, const maptype& weight){
